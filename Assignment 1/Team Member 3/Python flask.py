...
def check(email):
    url = "https://lframmp8cd.execute-api.ap-south-1.amazonaws.com/login?email=" + email
...
def register():
    ...
    print(x)
    params = "name=" + x[0] + "&email=" + x[1] + "&mobile=" + x[2] + "&city=" + x[3] + "&covid=" + x[4] + "&blood=" + x[
        5] + "&password=" + x[6]
...
@app.route('/stats')
def stats():
    url = "https://wlxyss47rc.execute-api.ap-south-1.amazonaws.com/getbloodcount"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    print(address)
    url = "https://84ky3abv1d.execute-api.ap-south-1.amazonaws.com/getpersonsgroup?blood=" + bloodgrp
    status = requests.request("GET", url)
    a = status.json()
    print(a)
    phone = []
    ...
    for i in a:
        url = "https://www.fast2sms.com/dev/bulk?authorization=xCXuwWTzyjOD2ARd1EngbH3a7tKIq5PklJ8YSf0Lh4FQZecs9iNI1dSvuqprxFwCKYJXA5amQkBE36Rl&sender_id=FSTSMS&message=" + msg + "&language=english&route=p&numbers=" + str(
            i['mobile'])
